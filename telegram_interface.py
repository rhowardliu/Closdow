...
def generate_keyboard_from_available_list(state):
    ...
    keyboard = []
    if state == STATES.OPERATE:
        keyboard += [["Open all windows", "Close all windows"]]
        for name, status in window_selections.items():
            if status:
                keyboard += [[name + "\nis opened. Click to close."]]
            else:
                keyboard += [[name + "\nis closed. Click to open."]]


    elif state == STATES.CHILDLOCK:
        for name, status in window_selections.items():
            if status:
                keyboard += [[name + "\nis Child locked. Click to disable lock."]]
            else:
                keyboard += [[name + "\nis not child locked. Click to enable lock"]]

    elif state == STATES.SENSORS:
        keyboard += [["Enable all sensors", "Disable all sensors"]]
        for name, status in window_selections.items():
            if status:
                keyboard += [[name + "\nSensors are enabled. Click to disable."]]
            else:
                keyboard += [[name + "\nSensors are disabled. Click to enable."]]
...
def command_wait(bot,update):
    command_wait_keyboard = [["Open/Close windows", "Set Time"],
                             ["Toggle Child Lock", "Toggle Sensors"],
                             ]
...
def operate_select_windows(bot,update):
    # STATE_SEQUENCE.append(STATES.OPEN)
    global window_selections
    window_selections = myWindowManager.get_windows(myWindowManager.ISOPEN)

    # create keyboard
    operate_windows_keyboard = generate_keyboard_from_available_list(STATES.OPERATE)
    markup = ReplyKeyboardMarkup (operate_windows_keyboard, one_time_keyboard= True)

    logger.info("Operate: Windows selection page")
    update.message.reply_text("Select windows to open or close.",reply_markup = markup)
    return STATES.OPERATE

def open_all_windows (bot, update):
    myWindowManager.change_all_window_state(True)
    update.message.reply_text("Opening all windows...")
    return main_menu(bot, update)

def close_all_windows (bot, update):
    myWindowManager.change_all_window_state(False)
    update.message.reply_text("Closing all windows...")
    return main_menu(bot, update)


def operate_execute(bot, update):
    ...
    if window_chosen in window_selections.keys():
        logger.info("Operating window: " + window_chosen)
        changed_state = not window_selections[window_chosen]
        myWindowManager.change_window_state(window_chosen, changed_state)
        if changed_state:
            update.message.reply_text(window_chosen + " is opening..")
        else:
            update.message.reply_text(window_chosen + " is closing..")
        return main_menu(bot, update)
...
def timeset_to_add_or_remove(bot, update, user_data):
    ...
    keyboard = [['Add a time to open/close window', 'Remove a time to open/close window']]
...
def enable_all_sensors(bot, update):
    ...
    update.message.reply_text("-WARNING: SENSORS OVERRIDE USER COMMANDS-")
    return main_menu(bot, update)
...
async def execute(windowManager):
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            STATES.COMMAND_WAIT: [RegexHandler('^Open/Close windows', operate_select_windows),
                                  RegexHandler('^Toggle Child Lock$', childlock_windows_selection),
                                  RegexHandler('^Set Time$',timeset_windows_selection),
                                  RegexHandler('^Toggle Sensors$', sensors_windows_selection)
                                  ],
            STATES.OPERATE: [RegexHandler('^Open all windows$', open_all_windows),
            ...
            STATES.TIME_EDITING: [RegexHandler('^Add a time to open/close window$',timeset_add),
                                  RegexHandler('^Remove a time to open/close window$', timeset_remove, pass_user_data= True)
                                  ],
